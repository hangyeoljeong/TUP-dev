...
export const saveUserInput = async (payload) => {
  const { data } = await http.post("/api/team1/save/", payload, {
...
export const getMatchedTeams = async () => {
  const { data } = await http.get("/api/team1/teams/");
  return data;
};
...
export const performFeedbackAction = async ({ teamId, userId, action }) => {
  const { data } = await http.post("/api/team1/submit_feedback/", {
    teamId,
    userId,
...
export const getWaitingUsers = async () => {
  const { data } = await http.get("/api/team1/waiting/");
